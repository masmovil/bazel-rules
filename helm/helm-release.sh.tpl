...
if [ "$FORCE_HELM_VERSION" == "v2" ] || ( [ "$FORCE_HELM_VERSION" != "v3" ] && [ $({KUBECTL_PATH} get pods -n {TILLER_NAMESPACE} | grep tiller | wc -l) -ge 1 ] ); then
    ...
    echo "{HELM_PATH} upgrade --install --tiller-namespace {TILLER_NAMESPACE} --kube-context {KUBERNETES_CONTEXT} --namespace {NAMESPACE} {VALUES_YAML} {RELEASE_NAME} {CHART_PATH}"
    {HELM_PATH} upgrade --install --tiller-namespace {TILLER_NAMESPACE} --kube-context {KUBERNETES_CONTEXT} --namespace {NAMESPACE} {VALUES_YAML} {RELEASE_NAME} {CHART_PATH}
else
    ...
    echo "{HELM3_PATH} upgrade {RELEASE_NAME} {CHART_PATH} --install --kube-context {KUBERNETES_CONTEXT} --namespace {NAMESPACE} {VALUES_YAML}"
    {HELM3_PATH} upgrade {RELEASE_NAME} {CHART_PATH} --install --kube-context {KUBERNETES_CONTEXT} --namespace {NAMESPACE} {VALUES_YAML}
fi
