...
if [ "$FORCE_HELM_VERSION" == "v2" ] || ( [ "$FORCE_HELM_VERSION" != "v3" ] && [ $({KUBECTL_PATH} get pods -n {TILLER_NAMESPACE} | grep tiller | wc -l) -ge 1 ] ); then
    ...
    {HELM_PATH} upgrade --install --tiller-namespace {TILLER_NAMESPACE} --kube-context {KUBECONTEXT} --namespace {NAMESPACE} {VALUES_YAML} {RELEASE_NAME} {CHART_PATH}
    ...
    echo "{HELM3_PATH} upgrade {RELEASE_NAME} {CHART_PATH} --install --kube-context {KUBECONTEXT} --namespace {NAMESPACE} {VALUES_YAML}"
    {HELM3_PATH} upgrade {RELEASE_NAME} {CHART_PATH} --install --kube-context {KUBECONTEXT} --namespace {NAMESPACE} {VALUES_YAML}
fi
